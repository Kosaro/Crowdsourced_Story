...
def upvote(request, plot_point, current_user):
    ...
    newUV = Upvote(user=current_user, plot_point=plot_point)
    newUV.save()
...
def choice_upvote(choice, current_user):
    ...
    newUV = Choice_Upvote(user=current_user, choice=choice)
    newUV.save()
...
def downvote(plot_point, current_user):
    ...
    newDV = Downvote(user=current_user, plot_point=plot_point)
    newDV.save()


def choice_downvote(choice, current_user):
    choice.dv = choice.dv + 1
    newDV = Choice_Downvote(user=current_user, choice=choice)
    newDV.save()
